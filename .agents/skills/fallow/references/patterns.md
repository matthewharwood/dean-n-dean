...
- [Guard `git push` for Codex](#guard-git-push-for-codex)
...
## Guard `git push` for Codex

Codex does not depend on a repository-local tool interception format. Use the
same Git hook for people and coding agents so the gate applies consistently.

Install Fallow's Git hook:
...
Also keep the agent-facing rule in the repository root `AGENTS.md`:
...
Keep `fallow audit` in CI alongside the local hook. The Git hook is a local
reinforcement layer; CI remains the shared source of truth.
